...
### MQTT topics
Command/state topics are under `spa_intex/` (e.g. `spa_intex/target`, `/thermostat`, `/filtration`, `/jet`, `/reset`; state published to `/info`). On connect, `publishMQTTDiscovery()` publishes Home Assistant MQTT auto-discovery configs under `homeassistant/...`. `spa_intex/reset` triggers a deferred (10 s) reboot via the non-blocking `resetRequested`/`resetTimer` mechanism in `loop()`. The thermostat is a single HA `climate` entity that reuses the `spa_intex/target` and `spa_intex/thermostat` command topics — so `/thermostat` accepts `1`/`0` (switch) or `heat`/`off` (climate mode). The legacy `number`/`switch` discovery configs are cleared with an empty retained payload so HA drops them; the climate payload is large, so `connect_mqtt()` raises `setBufferSize()` to 2048.
